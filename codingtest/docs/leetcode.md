...
- 접근방법
    ...
    - 섬의 개수 결과값 반환
### 2) [전화 번호 문자 조합](../code/graph/LetterCombinationsOfAPhoneNumber.kt)
- 접근방법
    - 조합이 가능한 모든 문자열 출력
    - 각 번호마다 해당되는 문자열 값에 스트링 리트스 만들기 mapOf()
    - mapNotNull() 널이 아닌 결과값만 목록으로 반환
    - 입력된 숫자값이 있는 값만 추출
    - reduce() 컬렉션의 데이터를 모으는 함수
    - flatMap 컬렉션을 하나로 합치는 연산
    - map 컬렉션 하나마다 값을 합침
### 3) [순열](../code/graph/Permutation.kt)
- 접근방법
    - 서로다른 정수 입력, 정수형 리스트가 담긴 리스트로 반환
    - 변수로 임시리스트 변수, 입력받은 리스트를 변수로 받음
    - flatMap으로 재귀함수로 임시리스트와 입력받은 리스트를 리스트로 변경
    - 순열을 다시호출 할땐 임시 리스트엔 number를 더하고 다른리스트엔 number 뺌
### 4) [조합](../code/graph/Combinations.kt)
- 접근방법
    - 재귀형태로 문제를 품
    - 끝나는 숫자와 조합카운트만큼 조합을 생성
    - 1부터 끝나는 숫자까지 재귀로 호출
    - 자기자신의 이전의 값을 고정하여 넘김
    - 순열과는 조합카운트만큼의 제약이 있음
    - 조합리스트에 1일때는 리스트에 값을 넣음
    - 1이 아닐때 재귀로 조합카운트를 1씩 줄이며 리스트에 값을 넣음
### 5) [조합의 합](../code/graph/CombinationSum.kt)
- 접근방법
    - 숫자 집합을 조합
    - 모든 중복조합을 찾아야함
    - dfs 재귀호출
    - 합을 갱신하는 변수, 자기자신을 포함하는 변수
    - 종료조건으로 합변수와 목표값이 같으면 리스트에 값 추가
    - 합이 목표값보다 크면 빠져나옴
    - 목표값보다 크지 않으면 pathList에 배열의 값을 집어 넣음
    - for문 안에 재귀함수를 호출해 모든 중복조합을 확인
### 6) [부분 집합](../code/graph/Subsets.kt)
- 접근방법
    - 숫자리스트 생성
    - 빈리스트를 집어 넣음
    - addAll()함수를 통해 리스트를 추가함
    - foreach문과 map을 통해 리스트에 값을 추가함
### 7) [일정 재구성](../code/graph/ReconstructItinerary.kt)
- 접근방법
    - 스케줄목록 결과 반환해주는 리스트 생성
    - 스케줄그래프를 HashMap으로 문자열과 우선순위 큐 생성
    - 반복문으로 조건 값 확인 후 우선순위 높을 것을 빼냄
    - 일정리스트에 값 추가
    - 어휘순 결과로 뒤집어서 결과 리스트 리턴
