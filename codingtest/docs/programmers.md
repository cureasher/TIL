...
### 10) [2016](../code/Array/DayCalculation.kt)
- 접근방법
    - 월과 일읍 입력받아 2016년의 요일 출력하는 문제
    - 2016년 1월 1일은 금요일
    - 입력한 월의 지난달의 일수와 입력한 일수를 더함
    - 일합한 값을 7로 나눠서 요일 값 리턴
### 11) [최소 직사각형](../code/Array/MinimumRectangle.kt)
- 접근방법
    - 명함지갑을 만들 명함의 가로와 세로 중 가장 큰 값을 구함
    - coerceAtLeast()함수로 사각형의 가장 큰 값 저장
    - 최대 가로와 최대 세로 높이를 곱한 값 반환
### 12) [모의고사](../code/Array/MockExam.kt)
- 접근방법
    - 학생들의 정답 순서 배열 만듦
    - 문제를 맞춘 학생의 수를 세는 배열 만듦
    - 학생의 정답과 시험의 정답을 비교하고 학생의 카운트를 셈
    - 문제를 맞춘 학생이 있으면 학생시험결과 배열에 넣음
    - 학생시험결과 배열 출력
## 2. 수학
...
### 10) [3진법 뒤집기](../code/StringControll/TernaryReverse.kt)
- 접근방법
    - 자연수를 입력받아 toString() 3진법으로 변환
    - 3진법 숫자를 reverse로 뒤집음
    - 뒤집은 3진법 숫자를 정수로 변환한 값 출력