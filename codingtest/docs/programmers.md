...
### 12) [부족한 금액 계산](../code/math/CalculatePrice.kt)
- 접근방법
    - 놀이기구 금액이 3, 6, 9, 12 2배씩 늘어남
    - 타고 싶은 횟수 * 가격 + 가격 * 횟수를 2로 나누고 현재 있는 금액 차감
### 13) [나머지가 1이 되는 수 찾기](../code/math/RemainOneNumber.kt)
- 접근방법
    - number를 2부터 매개변수 값까지 반복
    - 1은 모든 수를 나눌수 있기에 2부터 확인
    - 매개변수를 number로 나누어 나머지가 1인 결과값 반환
    - 나머지가 1이 될 수 없는 경우는 자기자신 값 반환
## 3. 문자열
